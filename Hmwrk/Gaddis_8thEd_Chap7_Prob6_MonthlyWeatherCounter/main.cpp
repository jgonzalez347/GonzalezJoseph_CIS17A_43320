 ...
 * Created on March 03, 2020, 10:00 AM
...
string stats(char data[][DAYS],char stat[][MONTHS]){
   ...
   //Added lines of Code to account for Duplicate Months of matching Rain Months  
    if(stat[0][2]==stat[1][2]&&stat[0][2]>stat[2][2])wchMnth.append(" & JULY");
    if(stat[0][2]==stat[2][2]&&stat[0][2]>stat[1][2])wchMnth.append(" & AUGUST");
    if(stat[1][2]==stat[2][2]&&stat[1][2]>stat[0][2])wchMnth.append(" & AUGUST");   
    if(stat[0][2]==stat[1][2]&&stat[0][2]==stat[2][2])wchMnth.append(" & JULY & AUGUST");
...
void wrtFile(fstream &out,char fn[]){
    const int SIZE=10;
    char weather[SIZE+1]="SSSSSSCCCR"; //60% for Sunny | 30% for Cloudy | 10% for Rainy
    out.open(fn,ios::out);
    for(int month=0;month<MONTHS;month++){
        for(int day=0;day<DAYS;day++){
